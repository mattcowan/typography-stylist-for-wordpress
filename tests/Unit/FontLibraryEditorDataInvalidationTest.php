...
class FontLibraryEditorDataInvalidationTest extends TestCase {
    ...
    public function test_saving_a_font_face_clears_the_font_css_caches_and_editor_data() {
        $plugin = $this->getPluginInstance();
        $plugin->on_wp_font_face_saved(902, $this->makePost('wp_font_face'), false);

        $this->assertCount(1, $this->editorDataDeletes());
        $fontCss = array_filter($this->queries, function ($sql) {
            return false !== strpos($sql, '_transient_typost_font_css_');
        });
        $this->assertNotEmpty($fontCss, 'clear_cache() must drop the combined font CSS transients');
    }
