...
- `typost_paragraph_styles` — array of `{id, name, created, modified, properties}` in wp_options. `properties` mirrors editor state keys: `fontId` (numeric), `fontWeight`, `fontStyle` (`'normal'`/`'italic'`/`'oblique'`; absent = inherit — style-owned since the italic fix, see below), `fontSize` (`'responsive'`, `'fit'`, or px value), `fontSizeMin/Preferred/Max`, `fitMaxSize` (fit styles only — always stored, 0 = uncapped), `letterSpacing` (thousandths of em), `lineHeight`, `features[]`, `fontVariationSettings`. When core gains a new text property, walk `docs/typography-property-checklist.md` — `fontStyle` shipped in core without these style-side integrations and silently dropped italic from every saved style.
...
- **In-session saves:** the server paths above print only at page load, and applying a style strips inline styles from the span/block (the class provides rendering) — so a style created or updated mid-session used to render as theme defaults until reload ("saved my style and the text went default-bold"). `editor.js` listens for `typost-paragraph-styles-updated` and rebuilds `<style id="typost-ps-dynamic-css">` in the top document **and** every `iframe[name="editor-canvas"]` from `buildStyleCssBlock()` in ps-utils — the JS twin of `generate_style_css()`. The panel dispatches that event *before* it dispatches the apply, so the rules exist by the time the span relies on them. **The two generators must stay byte-identical for the same properties — change both or neither.**
...
Pure logic lives in `assets/js/lib/ps-utils.js` (UMD-lite: `window.typostPSUtils` + CommonJS export) — `findFontName`, `isStyleModified`, `buildPropertiesFromState`, `buildApplyEventDetail`, `buildStylePreviewStyle`, `buildStyleCssBlock` — enqueued as a dependency of `editor.js` and covered by Jest in `__tests__/` (auto-collected by core `npm test`).
...
Optional direct-access button in the Typography Stylist block toolbar, registered on core's `typost_editor_toolbar_buttons` filter with `editors: ['qft']` when `typost_ps_toolbar_button` is on (default off; setting rendered into core's Options tab via `typost_admin_options_rows`, flag exposed as `typostData.paragraphStylesOptions.toolbarButton`). Its `PSIcon` is a swash "P" on a **square 256×256 grid**, drawn as one of a set with core's swash "T" and the Glyphs Panel swash "G" so the toolbar group shares a cap height and baseline; the source lives in core at `assets/images/icons/toolbar-p.svg`. (It replaced a "Ps" ligature that needed a viewBox wider than tall to hold its cap height — that mark is gone.)
