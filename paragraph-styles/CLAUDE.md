...
It applies with `dispatchApply(style, 'inspector')` — **not `'qft'`**: core drops a `'qft'` apply unless the Quick Feature Toggle modal is open, whereas an `'inspector'` apply targets the selected block, which is exactly the block whose toolbar was clicked. Apply and Detach both close the browser: they are terminal actions, closing returns focus to the toolbar button, and it avoids the focus bug where detaching unmounts the very button that was clicked and drops focus to the document root.
