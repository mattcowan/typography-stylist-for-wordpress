...
- **Rendered weight, not the `'400'` default (v2.3+)**: a core heading is bold because of theme CSS, while `fontWeight` defaults to `'400'` and save always emits it. Treating unset as 400 lightened converted headings *and* made Glyphs-panel insertions lighter than the surrounding headline, because consumers wrote the reported weight onto the span they created. `getEffectiveFontWeight()` is the single source: explicit `data-fontweight` on the selection → a weight the author picked in the popover (tracked by `_pendingChanges.keys`) → the computed weight. It feeds both `convertToBlock()` and the `typost_current_editor_state` filter. `getActiveFontWeight()` hard-defaults to `'400'`, so `getExplicitFontWeight()` exists to tell "unset" from "deliberately 400". The apply path follows the same idea (QA 2026-09 E-2): `resolveWeightToWrite()` in `assets/js/inline-apply-rules.js` writes `data-fontweight` only when the selection already stores a weight or the author picked one in this popover session (`_authorPickedWeight`); the select opens on `getEffectiveFontWeight()` for display only, so a feature toggle on a theme-bold heading never writes 400. `buildConvertBlockAttributes()` (same module, E-3) gives a partial-selection conversion inherit defaults on the block — the span keeps the selection's size, spacing and line height — and carries the modal state only for a whole-block conversion. The block-menu transform can't reach the DOM through its attributes-only signature, so it uses `detectBlockComputedWeight()` in `utils.js`, locating the source through the selection and bailing when more than one block is selected.
...
- **Editor vs Save rendering:** `save.js` uses the CSS class when `styleClass` is set (for the frontend). Since the 2026-09 QA fixes `edit.js` does the same: when `styleClass` names a style that exists (`findParagraphStyleByClass`), the content element carries `typost-styled {styleClass}` and `buildStyle()` emits inline declarations only for attributes that differ from the style (`stylePropertyOverrides`) — the attributes are the Inspector's working copy, so an in-progress edit previews inline while everything else renders from the class, and a fixed px size (a style saved from the inline editor) renders at all. On `typost-paragraph-styles-updated` every block using the style re-syncs its copy from the style. The editor iframe receives the class CSS via `enqueue_block_assets`; note WordPress prefixes theme editor styles with `.editor-styles-wrapper`, which is why the module's selectors are boosted to (0,6,0) (see `paragraph-styles/CLAUDE.md`).
- **Modal focus guard (2026-09):** `installModalFocusGuard()` (utils.js, installed once from `edit.js`) keeps focus inside any `typost-*` modal when RichText re-applies a selection after an apply/insert. Firefox moves the active element into the canvas iframe on that write *without firing any focus event in either document* and ignores `focus()` on the element it still records as focused, so the guard combines focusout/focusin listeners, a `focusin` listener attached on demand to the canvas document, a 100 ms poll that runs only while a plugin modal is open, and a "focus the frame first" fallback. Keyboard users were otherwise stranded outside an open dialog with Escape dead (QA finding SR-7).
- **Glyph insertion swap window (2026-09):** the Glyphs panel's alternates view inserts with `swap: true` only while the author keeps browsing the same character (`shouldSwapInsertion` in `glyphs-panel/assets/js/lib/insertion.js`); both insert handlers run the range through `adjustInsertionRangeForSwap` (utils.js) so a different character goes *after* the glyph that stayed selected for swapping, and an alternate picked right after a caret insertion replaces that glyph (QA finding GP-1).
- **Indirect font references (2026-09):** the frontend font scan applies `typost_content_font_ids` (HOOKS.md) so content styled only through a paragraph-style class still enqueues its font; `Typost::clear_font_detection_cache()` is public so the module can invalidate the per-post detection cache when a style changes.
- **Screen-reader rules for the plugin modals (2026-09, QA Session B):** (1) core's `Notice` pushes its content into the a11y live region the moment it mounts, which is the same tick a modal takes focus, so NVDA read a notice *before* the dialog's name — every notice that mounts with a modal passes `spokenMessage=""` and, when it matters (the word-boundary warning), is announced by a deferred `wp.a11y.speak()` ~800 ms later, after the title. (2) Scroll containers are Tab stops in Firefox and Chrome and a screen reader reads the whole container at that stop, so `typost-modal-content` / `typost-scrollable-content` carry `tabIndex={-1}` (their controls still scroll them into view); the same fix is applied to the font picker's suggestions `<ul role="listbox">` by a MutationObserver in `assets/js/font-picker.js`, because core renders it without a tabindex. (3) The draggable header is a mouse-only drag surface (no role, no tabindex); keyboard repositioning is the named "Move panel, use arrow keys" button beside Close, and the tips bar has its own "Dismiss tip" button instead of Notice's "Close". (4) The resize handles are `aria-hidden` mouse affordances (they had `role="slider"` with no value and no keyboard handler). (5) The Paragraph Styles browser focuses the active (else first) row on open; the toolbar click context's `state.selectionParagraphStyleId` (the `data-style-id` at the captured selection, from `parseInlineStylesAtCursor().styleId`) is what it marks active in selection scope, never the block's `paragraphStyleId`. (6) Both editors confirm before a paragraph style replaces the selection's own styling (`countInlineParagraphStyleConflicts()` on RichText formats for the inline editor, `countParagraphStyleConflicts()` on HTML for the block) and fire `typost-paragraph-style-apply-cancelled` on cancel so the module's panel drops its badge. (7) The inline modal's Paragraph Style panel has no select: "Browse styles…" opens the module's `ParagraphStylesBrowser` through `openBrowser({ editorSource: 'inline' })`, which applies through the `source: 'inline'` route and reports back via `onApplied`; the browser filters with `filterParagraphStyles()` and pages `BROWSER_PAGE_SIZE` (24) rows at a time.
...
- **Adopted fonts get the same treatment (2026-09, QA finding E-8):** an adopted entry has no `css_content`/`font_faces` of its own, so the plugin builds its `@font-face` rules from the family's `wp_font_face` posts (`Typost_Font_Library_Bridge::build_font_face_css_for_family()` → `build_font_face_rule()`, pure and allow-list sanitized: family, style, weight incl. `"100 900"` ranges, display, stretch, unicode-range, `src` string or array via `esc_url_raw()` with the format hint derived from the file extension; faces without a usable `src` are skipped). On the frontend they print only when the font is in use (its `font_id` is in the detected/forced IDs, the same signal as the kits) **and** WordPress will not print it (`adopted_entry_faces_printed_by_wordpress()`, matching on the first family of the entry's `font_family` stack); the slugs the plugin prints for are folded into `get_font_css_cache_key()` so activating the family later cannot serve stale CSS. The editor iframe (`typost-block-fonts`), the popover-preview handle (`typost-editor-fonts`) and the admin preview (`get_admin_font_css()`, also returned by `admin/refresh`) include every adopted font unconditionally, like the kits, so their static transient keys stay valid (a duplicate of a face WordPress also prints is harmless). Adopted entries that share a `font_id` or slug with an uploaded kit are skipped everywhere — the kit-reuse branch of `adopt_library_font()` already prints through the kit path. Deleting an adopted family through the Library UI is reported by `handle_deleted_post()` (entry untouched) so `on_wp_font_family_deleted()` clears the CSS caches; `save_post_wp_font_face` → `on_wp_font_face_saved()` runs `clear_cache()`, so a face added to an already-adopted family reaches the printed CSS on the next request, and `save_post_wp_font_family` → `on_wp_font_family_saved()` invalidates the per-user editor-data transients so a family installed in the Library is in the picker on the next editor load instead of after the hourly transient expires (QA E-16; the widened `on_wp_font_family_deleted()` does the same for deleted families that were never registered or adopted).
