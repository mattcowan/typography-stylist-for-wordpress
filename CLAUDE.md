...
- **Conversion preserves the rendered weight (v2.3+)**: a core heading is bold because of theme CSS, while `fontWeight` defaults to `'400'` and save always emits it — so converting silently lightened headings. Both routes now read the computed weight: the inline Convert button via `getBlockInheritedWeight()`, the block-menu transform via `detectBlockComputedWeight()` in `utils.js` (transforms only receive attributes, so the source block is found through the selection and skipped when more than one block is selected). Precedence: explicit `data-fontweight` on the selection → a weight the author picked in the popover (tracked by `_pendingChanges.keys`) → computed. `getActiveFontWeight()` hard-defaults to `'400'`, so `getExplicitFontWeight()` exists to tell "unset" from "deliberately 400".
